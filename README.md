...
[![Version](https://img.shields.io/badge/version-20260912.1.0-blue.svg)](https://github.com/heihei0299/pi-switch/releases)
...
**Build from source** (requires Node.js >= 20, Go 1.24+):
...
# proxy started with a --host that differs from the config is not covered), and only
# when the plan actually publishes one of pi-switch's own providers with a model.
# Bind the proxy to loopback, or use a client that can send Basic. The shared
# password is deliberately never written into models.json.
...
| 💡 **Built-in Presets** | OpenAI, Anthropic, DeepSeek — add profiles instantly. The list has one source (`ProviderPresets`) and is filtered by the same api capability rule the write doors use, so it can never offer an api those doors refuse (`google-generative-ai` is known but not proxyable, so there is no Google preset) |
