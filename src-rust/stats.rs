...
pub fn virtual_session_for(
    ...
    for sess in sessions.values() {
        ...
        let time_diff = if sess_ms > entry_ms {
            sess_ms - entry_ms
        } else {
            entry_ms - sess_ms
        };
    ...
    for entry in entries {
        ...
        if eff_id2 != "unlabeled" {
            ...
            if let Some(n) = eff_name2 {
                if detail.conversation_name.is_none()
                    || detail
                        .conversation_name
                        .as_deref()
                        .filter(|s| !s.is_empty())
...
pub fn aggregate_conversations_paged(
    ...
    aggregate_conversations_paged_with_source(
        entries,
        window,
        page,
        limit,
...
pub fn get_conversations_paged(
    ...
    if matches!(source, ConversationSource::SessionScan) {
        return aggregate_conversations_paged_with_source(
            &entries,
            window,
            page.unwrap_or(0),
            limit.unwrap_or(100),
...
mod tests {
    ...
    fn virtual_session_matches_with_provider_prefix_stripped() {
        ...
        assert!(
            hit.is_some(),
            "should match after stripping provider prefix"
        );
        assert_eq!(hit.unwrap().id, "sess-1");
        ...
        assert!(
            hit2.is_some(),
            "should match when entry has prefix and sess does not"
        );
    }
    ...
    fn aggregate_session_scan_virtual_groups_and_shows_title() {
        ...
        assert_eq!(
            stats.recent_requests[0].conversation_id.as_deref(),
            Some("sess-1")
        );
        assert_eq!(
            stats.recent_requests[0].conversation_name.as_deref(),
            Some("my title")
        );
    ...
    fn aggregate_proxy_preserves_original_and_off_empty() {
        ...
        assert_eq!(
            stats_proxy.by_conversation[0].name.as_deref(),
            Some("orig name")
        );
        // off should be empty
    ...
    fn aggregate_virtual_is_immutable_and_repeatable() {
        ...
        let entries = vec![e];
        let first = aggregate_paged_with_source(
            &entries,
            &HashMap::new(),
            60_000,
            now,
        ...
        let second = aggregate_paged_with_source(
            &entries,
            &HashMap::new(),
            60_000,
            now,
        ...
        assert_eq!(
            first.by_conversation[0].conversation_id,
            second.by_conversation[0].conversation_id
        );
    ...
    fn aggregate_conversations_paged_virtual_and_off() {
        ...
        let (rows, total) = aggregate_conversations_paged_with_source(
            &[e.clone()],
            None,
            0,
            50,
        ...
        let (rows_off, total_off) = aggregate_conversations_paged_with_source(
            &[e],
            None,
            0,
            50,
    ...
    fn aggregate_conversation_requests_virtual_filter() {
        ...
        let (rows, total) = aggregate_conversation_requests_with_source(
            &entries,
            "sess-1",
            0,
            50,
        ...
        let (rows2, total2) = aggregate_conversation_requests_with_source(
            &entries,
            "sess-1",
            0,
            50,
