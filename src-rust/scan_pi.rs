...
pub fn resolve_session_root_with(
    ...
    if let Some(v) = env_val {
        ...
        if !t.is_empty() {
            if is_relative_path(t) {
                log::warn!(
                    "PI_CODING_AGENT_SESSION_DIR is relative, requires project context: {}",
                    t
                );
    ...
    if let Some(v) = config_session_dir {
        ...
        if !t.is_empty() {
            if is_relative_path(t) {
                log::warn!(
                    "pi config sessionDir is relative, requires project context: {}",
                    t
                );
                return SessionRoot::RequiresProjectContext(t.to_string());
            }
...
fn parse_line_to_entry(line: &str) -> Option<RawEntry> {
    ...
    let parent_id = if parent_id.as_deref() == Some("") {
        None
    } else {
        parent_id
    };
...
mod tests {
    ...
    fn tmp_dir(name: &str) -> PathBuf {
        let base =
            std::env::temp_dir().join(format!("pi-switch-scan-{}-{}", std::process::id(), name));
    ...
    fn collect_flat_and_project_directories() {
        ...
        write_file(
            ...
            &format!(
                ...
                msg("m1", None, "2026-08-01T00:01:00Z", "user", "hello")
            ),
        );
        ...
        write_file(
            ...
            &format!(
                ...
                msg("m2", None, "2026-08-01T00:01:00Z", "user", "hi")
            ),
        );
        // non-jsonl ignored
    ...
    fn collect_skips_invalid_header() {
        ...
        write_file(
            ...
            &format!(
                ...
                header(
                    3,
                    "019f0000-0000-0000-0000-000000000003",
                    "/tmp",
                    "2026-08-01T00:00:00Z"
                )
            ),
    ...
    fn collect_skips_oversized_file() {
        ...
        write_file(
            ...
            &format!(
                ...
                header(
                    3,
                    "019f0000-0000-0000-0000-000000000004",
                    "/tmp",
                    "2026-08-01T00:00:00Z"
                )
            ),
    ...
    fn parse_skips_too_many_entries() {
        ...
        let mut content = header(
            3,
            "019f0000-0000-0000-0000-000000000005",
            "/tmp",
            "2026-08-01T00:00:00Z",
        );
        content.push('\n');
        ...
        big.push_str(&header(
            3,
            "019f0000-0000-0000-0000-000000000006",
            "/tmp",
            "2026-08-01T00:00:00Z",
        ));
        big.push('\n');
    ...
    fn active_branch_from_latest() {
        ...
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000010",
            "/proj",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn cycle_is_broken() {
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000012",
            "/proj",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn legacy_version_uses_linear_chain() {
        let h = header(
            1,
            "019f0000-0000-0000-0000-000000000013",
            "/proj",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn title_priority_session_info_over_user_message() {
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000014",
            "/proj/myapp",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn title_falls_back_to_cwd_basename() {
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000015",
            "/home/shial/myproj",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn title_truncation_and_sanitization() {
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000016",
            "/tmp",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn last_active_is_max_timestamp() {
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000017",
            "/tmp",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn invalid_id_is_skipped() {
        let h = header(
            3,
            "019f0000-0000-0000-0000-000000000018",
            "/tmp",
            "2026-08-01T00:00:00Z",
        );
    ...
    fn scan_sessions_integration_flat_and_nested() {
        ...
        let h1 = header(
            3,
            "019f0000-0000-0000-0000-000000000020",
            "/tmp/proj",
            "2026-08-01T00:00:00Z",
        );
        ...
        let h2 = header(
            3,
            "019f0000-0000-0000-0000-000000000021",
            "/home/shial",
            "2026-08-01T00:00:00Z",
        );
        ...
        assert_eq!(map.len(), 2);
        assert_eq!(
            map.get("019f0000-0000-0000-0000-000000000020")
                .unwrap()
                .title,
            "hello world"
        );
        assert_eq!(
            map.get("019f0000-0000-0000-0000-000000000021")
                .unwrap()
                .title,
            "my session"
        );
