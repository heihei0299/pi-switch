...
func TestChannelExpose_RequiresChannel(t *testing.T) {
	dir := t.TempDir()
	writeChannelConfig(t, dir, channelExposeConfig())
	r := NewMgmtRouter()
	if code := putExpose(t, r, "/api/profiles/sup/expose", `{"modelIds":["m1"]}`); code != 400 {
		t.Fatalf("no-param expose code = %d, want 400", code)
	}
}

func TestChannelExpose_RejectsLegacyTopLevelShape(t *testing.T) {
	dir := t.TempDir()
	writeChannelConfig(t, dir, `{"version":2,"profiles":{"sup":{"api":"openai-completions","responsesMode":"auto","baseUrl":"http://x","apiKey":"k","models":[{"id":"m1","contextWindow":128000,"maxTokens":16384}],"exposedModels":[]}},"settings":{}}`)
	r := NewMgmtRouter()
	if code := putExpose(t, r, "/api/profiles/sup/expose", `{"modelIds":["m1"]}`); code != 400 {
		t.Fatalf("legacy expose code = %d, want 400", code)
	}
}
