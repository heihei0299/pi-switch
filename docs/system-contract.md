...
2. 整文件门只拦运行期会被 `translator.PlanRequest` 拒绝的 effective api/mode 组合，包括：profile 与 channel 都没有 api、api 未知、以及**已知但当前不可代理**（`protocol.CanProxy == false`，如 `google-generative-ai`——`translator.upstreamFormat` 对它的每个请求都会失败）。capability 判定只来自 `protocol.CanProxy`，不维护第二份列表；`GET /api/presets` 同样按它过滤，避免提供必然保存失败的预设。判定以 `ResolvedUpstreams()` 为单位：它为空（profile 没有 channel 且没有 baseUrl/apiKey/headers）而 profile 自己声明了 api 时，按 profile 顶层 api 判一次——这与 Profile CRUD 门、advisory 门是同一条规则（`ValidateEffectiveFlatAPI`）。唯一剩下的分歧是**连 api 都没有声明**的 profile：它没有任何可判的组合，整文件门放行，CRUD 门与 advisory 门仍拒收/报错。
