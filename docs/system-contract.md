...
| Profile CRUD（`POST /api/profiles`、`PUT /api/profiles/:name`、CLI `provider add`） | 跑完整 `profile.ValidateProfile`：responsesMode → 无 channel 的 flat profile 判定 effective api（缺失/未知/已知但不可代理；与 advisory 门同一条规则，与整文件门的分歧见第 2 条）→ profile shape → retry。profile 声明了 channel 时，顶层 api 不参与运行期解析，逐 channel 由 `ProfileIssues`/`ValidateUpstreamAPI` 判定 |
...
2. 整文件门只拦运行期会被 `translator.PlanRequest` 拒绝的 effective api/mode 组合，包括：profile 与 channel 都没有 api、api 未知、以及**已知但当前不可代理**（`protocol.CanProxy == false`，如 `google-generative-ai`——`translator.upstreamFormat` 对它的每个请求都会失败）。capability 判定只来自 `protocol.CanProxy`，不维护第二份列表；`GET /api/presets` 同样按它过滤，避免提供必然保存失败的预设。判定以 `ResolvedUpstreams()` 为单位：profile 没有 channel 且没有 baseUrl/apiKey/headers 时它返回 nil，这条 profile 就不进判定——该形状没有任何 channel，route resolution 与 `/v1/models` 也不会遍历它；同一形状下 Profile CRUD 门与 advisory 门仍按 profile 顶层 api 判定并拒收，这是两门之间有意保留的差异（见第 5 条）。
