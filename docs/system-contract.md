...
| `PUT /api/config`（整文件写入） | 按运行期同一口径逐个 channel 判定 effective api/responsesMode（channel 声明优先、否则回退 profile；legacy flat profile 走 `ResolvedUpstreams` 的 fallback 语义，因此它的 profile 级 api/baseUrl/apiKey 也参与判定；`ResolvedUpstreams()` 为空但 profile 声明了 api 时按该 api 判，见第 2 条）；被拒绝的 effective api 有三类：缺失、未知、已知但当前不可代理。不跑 shape/模型/channel 名校验 |
| Profile CRUD（`POST /api/profiles`、`PUT /api/profiles/:name`、CLI `provider add`） | 跑完整 `profile.ValidateProfile`：responsesMode → 无 channel 的 flat profile 判定 effective api（缺失/未知/已知但不可代理；与 advisory 门、整文件门同一条规则）→ profile shape → retry。profile 声明了 channel 时，顶层 api 不参与运行期解析，逐 channel 由 `ProfileIssues`/`ValidateUpstreamAPI` 判定。与另外两个门的两处分歧见第 2 条 |
| Profile duplicate（`POST /api/profiles/:name/duplicate`、CLI `provider duplicate`） | 逐字复制磁盘上既有的 profile。复制前按整文件门同一顺序判两件事：profile 顶层 api/responsesMode 自洽（api 为空时与门一样跳过；这就是第 4 条「保留的 profile 顶层组合检查」，channel 覆盖它不豁免），随后 `config.ValidateResolvedCapability`。因此**副本不会是 `PUT /api/config` 因 mode/capability 而拒绝的配置**；shape、模型、channel 名与 retry 仍不阻止复制（带这些问题的源要能「复制一份再改」）。它不新增内容，所以 CRUD 门「每个 channel 必须自带 api」那条不适用于它 |
...
2. 整文件门只拦运行期会被 `translator.PlanRequest` 拒绝的 effective api/mode 组合，包括：profile 与 channel 都没有 api、api 未知、以及**已知但当前不可代理**（`protocol.CanProxy == false`，如 `google-generative-ai`——`translator.upstreamFormat` 对它的每个请求都会失败）。capability 判定只来自 `protocol.CanProxy`，不维护第二份列表；`GET /api/presets` 同样按它过滤，避免提供必然保存失败的预设。判定以 `ResolvedUpstreams()` 为单位，收在 `config.ValidateResolvedCapability`：逐个 channel 判 effective api（channel 声明优先、否则回退 profile），`ResolvedUpstreams()` 为空（无 channel 且无 baseUrl/apiKey/headers）时按 profile 顶层 api 判一次——`ValidateEffectiveFlatAPI` 就是这一格，Profile CRUD 门与 advisory 门共用同一条。duplicate 门复制前调的也是 `ValidateResolvedCapability`，所以副本与这道门口径一致。
   剩下**两处**有意差异，都在能力维度之外：(i) **连 api 都没声明**的 profile 没有任何可判的组合，整文件门与 duplicate 放行，CRUD 门拒收、advisory 报错；(ii) profile 顶层没有 api 而 channel 自带 api 时，两个写门按 channel 的 effective api 放行，advisory 仍按 profile 内容报 `api required`（它比写门更严，见下一行的 advisory 行）。
