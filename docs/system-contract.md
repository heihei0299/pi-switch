...
2. 整文件门只拦运行期会被 `translator.PlanRequest` 拒绝的 effective api/mode 组合，包括：profile 与 channel 都没有 api、api 未知、以及**已知但当前不可代理**（`protocol.CanProxy == false`，如 `google-generative-ai`——`translator.upstreamFormat` 对它的每个请求都会失败）。capability 判定只来自 `protocol.CanProxy`，不维护第二份列表；`GET /api/presets` 同样按它过滤，避免提供必然保存失败的预设。
