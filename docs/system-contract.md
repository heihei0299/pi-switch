...
| Profile duplicate（`POST /api/profiles/:name/duplicate`、CLI `provider duplicate`） | 逐字复制磁盘上既有的 profile：不跑 shape/模型/channel 名/retry（带这些问题的源仍要能「复制一份再改」），但复制前跑能力判定 `config.ValidateResolvedCapability`——与整文件门同一条，因此**副本不可能是整文件门会拒绝的配置**。它不新增内容，所以 CRUD 门「每个 channel 必须自带 api」那条不适用于它 |
