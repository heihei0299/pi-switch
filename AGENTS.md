...
代码实现默认遵循 Ponytail `full` 原则：

1. 先理解真实调用链，再决定实现。
2. 优先复用项目已有代码。
3. 优先标准库和平台原生能力。
4. 不为单一调用方创建抽象层。
5. 不增加解决当前问题不需要的扩展点、配置项或依赖。
6. 在满足正确性、安全性和明确需求的前提下，保持最小 diff。

Ponytail 不替代行为路由、CodeGraph、diagnose-fix、TDD 或领域建模。

`ultra` 仅在用户明确要求大幅精简、删除过度设计或进行复杂度清理时使用。
## 路由
